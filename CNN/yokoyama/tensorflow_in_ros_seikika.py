...
class RosTensorFlow():
    def __init__(self):
        ...
        self.sub_joy0_flag=0
        self._cv_bridge = CvBridge()
        self._sub = rospy.Subscriber('/camera/depth/image_rect_raw', Image, self.callback_image)
        self._sub2 = rospy.Subscriber('/joy', Joy, self.callback_joy)

        #self._pub = rospy.Publisher('/dx/cmd_vel_in', Twist, queue_size=10)#TODO
        self._pub2 = rospy.Publisher('/ypspur_ros/cmd_vel', Twist, queue_size=10)#TODO
        ...
        self._saver.restore(self._session, "model/1105test2.ckpt")

    def callback(self):

       #if(self.sub_image0_flag==1 & self.sub_joy0_flag==1):
        if(self.sub_image0_flag==1):
            ...
            #twist.linear.x = ac
            twist.linear.x = vel
            #twist.angular.z = stre.axes[2]
            #self._pub.publish(twist)
            self._pub2.publish(twist)
