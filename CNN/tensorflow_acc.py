...
class RosTensorFlow():
    def __init__(self):
        ...
        self._saver.restore(self._session, "model/0221-0-0-1.ckpt")
    ...
    def callback(self):
         ...
         if(self.sub_image0_flag==1):
             ...
             vel_list = [[vel]]
             self._twist = self._session.run(self.y_conv, feed_dict={self.x:self.cv_image1, self.vel:vel_list})

             image_steer = self._twist[:,0]
             image_accel = self._twist[:,1]
             twist = Twist()
             twist.linear.x = vel + image_accel
