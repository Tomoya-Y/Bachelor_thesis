...
class RosTensorFlow():
    def __init__(self):
        ...
        self._sub1 = rospy.Subscriber('/imagedayo', Image, self.callback_image, queue_size=1)
        self._sub2 = rospy.Subscriber('/ypspur_ros/cmd_vel', Twist, self.callback_twist, queue_size=1)
        self._pub1 = rospy.Publisher('/tomoyakun1', Twist, queue_size=1)#TODO
        ...
        self._saver.restore(self._session, "model/0130-11-2-0.ckpt")

    def callback(self):

         if(self.sub_image0_flag==1 and self.sub_twist0_flag==1):
             self.sub_image0_flag=0
             self.sub_twist0_flag=0
             list_image = []
             ...
             image_steer = self._steer
             twist = Twist()
             twist.linear.x = 0.3
             twist.angular.z = image_steer
             self._pub1.publish(twist)
             self._pub2.publish(self.twist_msg0)
             print image_steer
