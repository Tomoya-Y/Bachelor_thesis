...
MyLabeling::MyLabeling()
{
  ...
  image_pub = it.advertise("/image_", 1);
}
...
void MyLabeling::scan_callback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  else
  {
    ...
    cv::Mat image(150, 150, CV_8UC1, cv::Scalar::all(50));
    float theta_odom;
    float lotation[2][2];
    ...
    for(int i=0; i<40; i++)
    {
      ...
      {
        matrix[0][k] = (after_lotation[0][k] + fstack_odom.element[39].odom_list[0])*20;
        matrix[1][k] = (after_lotation[1][k] + fstack_odom.element[39].odom_list[1])*20;
      }
      for(int j=0; j<1081; j++)
      {
        if ((0 < (1*matrix[0][j])+120) && ((1*matrix[0][j])+120 < 150) && (0 < (-1*matrix[1][j])+75) && ((-1*matrix[1][j])+75 < 150))
        {
          image.at<uchar>((1*matrix[0][j])+120, (-1*matrix[1][j])+75) = color;
        }
      }
    }
...
{
  ros::init(argc, argv, "my_labeling5");
